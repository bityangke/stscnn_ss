function stats = getDatasetStatisticsStscnn(imdb, data_path)
...
for t=1:numel(train)
  ...
  img_list = dir(sprintf(imdb.paths.image, imdb.images.name{train(t)}));
  filename = img_list(1).name;
  rgb = imread(fullfile(data_path, filename)) ;
  rgb = single(rgb) ;
